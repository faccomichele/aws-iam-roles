resource "aws_iam_role_policy" "aws-simple-saml-idp-lambda" {
  count = contains(var.gha-roles, "aws-simple-saml-idp-lambda") ? 1 : 0

  name = "aws-simple-saml-idp-lambda-inline"
  role = aws_iam_role.gha-role["aws-simple-saml-idp-lambda"].name
    ...
    Statement = [
      {
        ...
        Resource = [
          ...
          "arn:aws:iam::${data.aws_caller_identity.current.account_id}:role/saml-role-${local.environment}-*",
        ]
      },
      {
